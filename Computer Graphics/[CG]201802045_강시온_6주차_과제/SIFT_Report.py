...
def SIFT(src):
    ...
    for i in range(len(keypoints)):
        ...
        for hist_index in range(len(orient_hist)):
            if orient_hist[hist_index] > np.max(orient_hist) * 0.8 and hist_index != np.argmax(orient_hist):
                keypoints.append(KeyPoint(x, y, None, float(hist_index*10), keypoints[i].response, 0, -1))
    ...
    for i in range(len(keypoints)):
        ...
        for row in range(-8, 8):
            for col in range(-8, 8):
                ...
                ###################################################################
                gaussian_weight = np.exp((-1 / 16) * (row ** 2 + col ** 2))
                new_angle = (angle[p_y, p_x] - keypoints[i].angle + 360) % 360
                descriptors[i, ((row+8)//4*4 + (col+8)//4)*8 + int(new_angle // 45)] += magnitude[p_y, p_x] * gaussian_weight
...
def main():
    ...
    # 결과의 학번 작성하기!
    cv2.imshow('my_sift - 201802045', result)
    cv2.waitKey()
